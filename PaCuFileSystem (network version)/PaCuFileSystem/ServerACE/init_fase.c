...
int init_message(int connsd)
{
	...
	if(msg_i->pay_desc == _INIT_WHOIS_PR )
	{	
		int b;
		b = op_init_whoIsPr(connsd , msg_i);
		free(msg_i);
		return 1 ;
	}	
...
int op_init_update(int connsd)
{
	...
	{
		free(msg);
		lock_WR_Reader(sem_list_dom_serv);
		if( send_dom_list(connsd , string_dom_serv) )
		{		unlock_WR_Reader(sem_list_dom_serv);
				set_timeout(connsd  , timeout_init_msg  , 0  , timeout_init_msg  , 0 );
				return 1;	
		}		
		else
...
// operazione sul messaggio di init_state_transaction, ritorna 1 se l'operazione è riuscita, 0 se c'e' qualche errore
// Parametri -> descrittore della connessione, messaggio ricevuto
int op_init_stateTR(int connsd , InitMessage *msg_i)
{
	if(msg_i->opt1 < 0 || msg_i->opt2 < 0 )
	{
		DiagMessage * msg;
		msg = init_DiagMessage( _DIAG_ERROR , _BIZANTINE_ERROR);
		send_DiagMessage(connsd , msg);
	...
	log = select_detTR(PATH_DB_LOG, "Log", msg_i->opt1,msg_i->opt2, 0, "\0", 0 , "\0", stringa_errore, &rc);
	...
	DiagMessage * msg;
	msg = init_DiagMessage( _DIAG_ACK , state);
	send_DiagMessage(connsd , msg);
	free(msg);
...
// operazione sul messaggio di  InitMessage Who Is Primary, ritorna 1 se l'operazione è riuscita, 0 se c'e' qualche errore
// Parametri -> descrittore della connessione, messaggio ricevuto
int op_init_whoIsPr(int connsd , InitMessage *msg_i)
{
	char temp_ip[_DIM_IPV4];
	char temp_name[_DIM_NAME_SERVER+1];
	int temp_port;
