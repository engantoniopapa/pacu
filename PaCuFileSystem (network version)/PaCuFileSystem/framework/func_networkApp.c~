...
//  Crea la connessione, restituisce: -1 se l'operazione ha avuto esito negativo, il socket se l'operazione ha avuto successo
...
//  Crea la connessione, restituisce: -1 se l'operazione ha avuto esito negativo, il socket se l'operazione ha avuto successo
//  Parametri -> ip e porta a cui connettersi
int connect_ServerServer(char *ipServer , uint16_t port_server , int n_prove)
{
	int			sockfd;
  struct sockaddr_in	servaddr;
	int i ;
...
//	Invio messaggio PR, ritorna: 0 se c'e' un errore, 1 se l'operazione va a buon fine
//	Parametri-> descrittore della connessione, messaggio da inviare
int send_PR_Message(int connsd , PR_Message* msg )
{
	int rc;
	int b;

	if( (rc = send(connsd,msg,sizeof(PR_Message),MSG_NOSIGNAL) 	)   ==  0)
	{
		b = 0;
	}
...
//	Ricezione messaggio di PR, ritorna: 0 se c'e' un errore, 1 se l'operazione va a buon fine
//	Parametri-> descrittore della connessione, puntatore al messaggio da ricevere
int  recv_PR_Message( int connsd , PR_Message* buffer)
{
	int rc;
	int b;
	memset(buffer, 0 , sizeof(PR_Message ) );

	if (   (rc = recv(connsd,buffer,sizeof(PR_Message),0) )   ==  0)
	{
		b = 0;
	}
