...
// Parametri fd file GA da leggere, descrittore del file con il capitolo, file destinazione ,posizione del capitolo,  cartella file temp
...
// modifica un capitolo di un file GA 
// ritorna: 1 se l'operazione è stata eseguita , 0 se c'e' qualche errore
// Parametri fd file GA da leggere , descrittore del file con il capitolo, percorso del file risultante, id del capitolo, cartella file temp
int mod_CapGA(int fd, int fc , char* result_file , int id_cap , char* tmp_path)
{
	char cap[1024]={0x0};
	char tmp[1024]={0x0};
	char *occurence;
	...
	FILE *file_d = fdopen(fd, "r");
	FILE *file_c = fdopen(fc, "r");
	snprintf(cap, 1024, "%s%d", CHAPTERS, id_cap);

	fseek(file_d, 0, SEEK_SET);
	fseek(file_c, 0, SEEK_SET);
	
	int dim_res = strlen(tmp_path) + L_TEMP;

	int fs = create_fileTemp(tmp_path , result_file , dim_res);
	
	while(file_d!=NULL && fgets(tmp, sizeof(tmp), file_d)!=NULL)
	{
		if ((occurence = strstr(tmp, cap)))
		{
			writen(fs, tmp, strlen(tmp));
			b = 1;
			break;
		}
		else
	...
	while(file_c!=NULL && fgets(tmp, sizeof(tmp), file_c)!=NULL)
	{
		writen(fs, tmp, strlen(tmp));	
	}
...
int anal_ga(int fd) 
{
	...
	while(file1!=NULL && fgets(tmp1, sizeof(tmp1), file1)!=NULL)
	{	
		if ((occurence = strstr(tmp1, CHAPTERS) ) || (occurence = strstr(tmp1, "&&&") ) || (occurence = strstr(tmp1, "è") ) || (occurence = strstr(tmp1, "é") ) || (occurence = strstr(tmp1, "à") ) || (occurence = strstr(tmp1, "ì") ) || (occurence = strstr(tmp1, "ò") )|| (occurence = strstr(tmp1, "ù") ))
		{
			fseek(file1, 0, SEEK_SET);
			return 0;
		}	
