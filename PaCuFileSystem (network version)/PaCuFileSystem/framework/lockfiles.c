...
struct list_el *lockLogicalWrite(int file, int init , int n_thread , regions * list_f , pthread_mutex_t p_mux  )
{
   ...
   if(list_f[index].inode != buf.st_ino) // se non è inizializzato
   {
...
int lockAllChapters(int file , regions * list_f  , int n_thread, pthread_mutex_t p_mux)
{
	...
	int temp_int;
	int alto = len ; /* lent è il numero degli elementi del vettore da ordinare */
	int i ;
...
// Sblocca tutti i capitoli di un file, un lock logico e fisico, ritorna 1 se l'operazione è andata a buon fine, se c'e' un errore 0
// descrittore del file ga
