...
// avvia l'operazione di recovery
void start_recovery(char *path)
{
	TrLog *rec_tr;
	...
	if( !update_log_state(path , LOG_PREPARA  , LOG_ABORT) )
	{
	...
	rec_tr = select_recTR(path , "Log" , str_cond , str_groupby , stringa_errore , &len);
	...
	else
	{
		...
		for( i = 0; i < len ; ++i)
		{
			if( ( sockfd = connessioneClient(rec_tr[i].ip ,  (uint16_t)rec_tr[i].port , 3) ) > 0 ) 
			{
				InitMessage* msg;
				msg = init_InitMessage( _INIT_STATE_TR , NULL , 0 , rec_tr[i].id_p , rec_tr[i].id_c);
				...
				if(send_InitMessage( msg , sockfd ))
				{
					...
					if(recv_DiagMessage(sockfd , msgD) )
					{
						...
						{
							if(update_log(path , rec_tr[i].id_p , rec_tr[i].id_c  , msgD->param_state) )
								printf("\e[1;32mAggiornamento del log [%d:%d] riuscito\033[m\n" ,rec_tr[i].id_p , rec_tr[i].id_c);
							if( rec_tr[i].type == _TR_RM_CAP || rec_tr[i].type == _TR_MV_CAP || rec_tr[i].type == _TR_MOD_CAP )
							{
								printf("\e[1;32mIl File: %s potrebbe non essere aggiornato\033[m\n" ,rec_tr[i].local_res);
							}	
						}	
						if(msgD->pay_desc == _DIAG_ERROR)
						{
							delete_log(path , rec_tr[i].id_p , rec_tr[i].id_c ) ;
						}
					}
					free(msgD);
				}
			...
			{
				if(delete_log(path , rec_tr[i].id_p , rec_tr[i].id_c ) )
					printf("\e[1;31mIl server a cui chiedere infromazioni sul log [%d:%d] è irrangiungibile \033[m\n" ,rec_tr[i].id_p , rec_tr[i].id_c);
			}
			close(sockfd);			
		}
