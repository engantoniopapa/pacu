...
void set_Unit_TH_Pong(Unit_TH_Pong *b , ListPong *l, Two_rooms *s, char *ip , int port	)
{
	b->list = l;
	b->sem = s;
	b->ip = ip;
...
void *thread_rcvPP_UDP(void *l )
{
  ...
  while (1) 
  {
  	...
  	if(exit_th_pp)
		...
		rcv = recvfrom(sockfd, buff_rcv, sizeof(PPMessage), 0, NULL, NULL) ;
		...
		else
		{
			if( buff_rcv->pay_desc == _PING_MESS_UDP || buff_rcv->pay_desc == _PONG_MESS_UDP )
			{
				...
				if( buff_rcv->pay_desc == _PONG_MESS_UDP)
				{
					...
					{
					  perror("\e[1;31mErrore: Nell'elaborazione del messaggio di pong \n");
					}
				}
			}
			else
			{
				puts("\e[1;31mErrore: Messaggio UDP non formattato  \n");
			}
