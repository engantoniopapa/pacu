...
void stampa_comandi_user()
{
	...
	printf(" %snewcap  \033[m crea un capitolo nel file GA (es: newcap <remote pathfile> <pos cap>)\n" , color); 
	printf(" %smvcap  \033[m sposta un capitolo del file GA (es: mvcap <local pathfile> <remote pathfile> <pos cap> <new pos cap> )\n" , color); 
	printf(" %smodcap  \033[m modifica un capitolo (es: modcap <local pathfile> <remote pathfile>  <max cap da modificare> (max %d capitoli nella transazione)\n" , color , tr_max_cap); 
	...
	while(b)
	{
		...
		switch (scelta) 
		{
			...
			case _SHELL_RMCAP: 
			{
				...
				{
					printf("errore nell'usare il comando rmcap \n");
				}
				else
				{
					char errore[_MAX_STRING_ERROR];
...
int op_cd(char *arg , Shell_El *shell)
{
	...
	else
	{
		...
		while(count_prove < 3 && true_con == 0 && strlen(shell->ip) > 0)
		{
			if( ( sockfd = connessioneClient(shell->ip ,  shell->port , 3) ) <= 0 ) 
			{
				count_prove++;
				sleep(2 * count_prove);
			}
	...
	if(true_con == 0 )
	{
		...
		while(b)
		{
			index_server = generatoreRandom(len_list_server);
							...
							//printf("ip----%s --port %d \n" , list_server[index_server].ip ,  list_server[index_server].port );
				...
				{				 
					...
					{					
						if(param)
						{
							...
							if(sd == -1)
							{
								printf("\e[1;31mErrore nella creazione del file: %s \033[m\n" , name_file );
								b = 0 ;
							}
							else
							{
								...
								if(write_res_on_res(fd  , sd,  max_line) )
								{
										close(fd);
										close(sd);
										printf("\e[1;32mFile scaricato correttamente :) \033[m \n\n");
										b = 1;
								}
								else
								{
							...
							{	
								...
								else
								{
									...
									if(pid == 0)
									{
										...
										printf("Inserisci il nome di un programma per aprire il file: ");
										fgets( comando , 100 , stdin);
										printf("\n");
										comando[strlen(comando)-1] = '\0';
										//snprintf( comando , 500*sizeof(char)," cat %s  " , realpath(PATH_GA_TXT, NULL) );
										//execlp( "xterm" , "xterm" , "-e ", comando ,NULL);
									 	
									 	execlp( comando , comando , realpath(PATH_GA_TXT, NULL) , NULL);								
									}
									waitpid(pid , stato , 0);
...
int op_transaction( int op , TokenArgs *list_arg , Shell_El *shell , char *errore )
{
	...
	if(op == _SHELL_RMCAP || op == _SHELL_MODCAP || op == _SHELL_UPLOAD || op == _SHELL_MVCAP)
	{
		...
		if(op == _SHELL_RMCAP || op == _SHELL_UPLOAD || op == _SHELL_MVCAP)
		{
			int i;
